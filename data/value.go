...
var (
	...
	ZXCMultipler  = newValue(true, false, ZXCPrecision, 0)
)
...
func init() {
	...
	if err := ZXCMultipler.canonicalise(); err != nil {
		panic(err)
	}
}
...
// number is interpreted as ZXC. If no decimal is used, it is interpreted as drips.
...
// Ratio returns the ratio a/b. ZXC are interpreted at face value rather than drips.
...
func (a Value) Ratio(b Value) (*Value, error) {
	...
	if num.IsNative() {
		...
		num, err = num.Divide(*ZXCMultipler)
		if err != nil {
			return nil, err
		}
	}
	if den.IsNative() {
		...
		den, err = den.Divide(*ZXCMultipler)
		if err != nil {
			return nil, err
		}
	}
...
// represented as decimal ZXC rather than drips.
func (v Value) String() string {
	...
	rat := v.Rat()
	if v.IsNative() {
		rat.Quo(rat, big.NewRat(int64(ZXCPrecision), 1))
	}
